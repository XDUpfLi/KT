...
from config import DATA_PATH_MINI
...
class MiniImagenetHorizontal(Dataset):
    def __init__(self, split, small=True):
        self.path = os.path.join(DATA_PATH_MINI, 'hsplit_{}'.format(split))
        self.files = []
...
class TieredImagenetHorizontal(Dataset):
    def __init__(self, split, small=True):
        self.path = os.path.join(DATA_PATH_TIER, 'hsplit_{}'.format(split))
        self.files = []
...
class ImagenetBasedDataset(Dataset):
    def __init__(self, split, small=True, tier=False, horizontal=False):
        if tier == True:
            self.path = os.path.join(DATA_PATH_TIER, 'test')
        else:
            self.path = os.path.join(DATA_PATH_MINI, 'test')
        self.files = []
        self.label2ind = {}
