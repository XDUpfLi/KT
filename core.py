...
class EvaluateFewShot(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for batch_index, batch in enumerate(self.taskloader):
            ...
            ws.append(w)
            seen += y_pred.shape[0]
            totals[self.metric_name] += torch.sum(y.cpu() == y_pred.cpu()).numpy()
            episode_acc_list.append(torch.sum(y.cpu() == y_pred.cpu()).numpy()/y_pred.shape[0])
            print("episode {} done!".format(batch_index+1))
        torch.cuda.set_rng_state(rng_state, self.kwargs['device'])
        acc = totals[self.metric_name] / seen
        logs[self.metric_name] = acc
        logs["eval_function"] = self.eval_fn.__name__
        test_acc, h = mean_confidence_interval(episode_acc_list)
        logs["h"] = h
