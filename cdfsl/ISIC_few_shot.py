...
class SetDataset:
    def __init__(self, batch_size, transform):
        ...
        for cl in self.cl_list:
            ...
            self.sub_dataloader.append(torch.utils.data.DataLoader(sub_dataset, **sub_data_loader_params)) # 长度为7个类别的能够迭代获得类中10个样本-标签对的数据加载器
        pass
...
class SetDataManager(DataManager):
    ...
    def get_data_loader(self, aug):
        transform = self.trans_loader.get_composed_transform(aug)
        dataset = SetDataset(self.batch_size, transform)  # 获得每类数据和标签的数据加载器，七类对应七个-list
        sampler = EpisodicBatchSampler(len(dataset), self.n_way, self.n_eposide)  # 5way采样器
